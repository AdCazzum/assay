...
import { STEP_LABEL, STEP_ORDER } from './events.js';
...
describe('render (happy path)', () => {
  ...
  it('renders real artifacts: tx ids, hashscan links, block numbers, claim values', () => {
    ...
    expect(output).toContain('atBlock: 21050112');
    expect(output).toContain('claim hasActiveMintRole: false');
  });
...
describe('render (slash sequence, the climax)', () => {
  ...
  it('shows the verifier verdict as FALSE, with both the claimed and actual value', () => {
    ...
    expect(output).toContain('hasActiveMintRole = false');
    expect(output).toContain('hasActiveMintRole = true');
  });
